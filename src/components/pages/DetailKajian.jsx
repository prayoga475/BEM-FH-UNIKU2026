...
export default function DetailKajian() {
  ...
  const artikel = kajianData.find((item) => String(item.id) === String(id));
  ...
  if (!artikel) {
    return (
      <div className="min-h-screen bg-[#F7F5F0] pt-32 pb-16 px-4 text-center">
        ...
        <p className="text-slate-500 mt-2">Maaf, kajian yang kamu cari tidak tersedia.</p>
        <Link
          to="/kajian"
    ...
    <div className="min-h-screen bg-[#F7F5F0] pt-28 pb-16 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="space-y-2">
          ...
          {/* Sumber Liputan (Hanya muncul jika properti sumber diisi pada data) */}
          {artikel.sumber && (
            <div className="flex items-center gap-1.5 text-xs sm:text-sm text-slate-500 px-1 italic">
              <ExternalLink className="w-3.5 h-3.5 text-slate-400" />
              <span>Photo Sumber: 
                <a
                  href={artikel.linkSumber}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-[#D97706] hover:underline font-medium not-italic">
                    <strong className="text-slate-700 not-italic">{artikel.sumber}</strong></a></span>
            </div>
          )}
        </div>
